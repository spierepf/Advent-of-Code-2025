...
impl Cell {
    pub fn adjacent_cells(&self) -> [Cell; 8] {
        [
            Cell { row: self.row + -1, col: self.col + 0 },
            Cell { row: self.row + -1, col: self.col + 1 },
            Cell { row: self.row + 0, col: self.col + 1 },
            Cell { row: self.row + 1, col: self.col + 1 },
            Cell { row: self.row + 1, col: self.col + 0 },
            Cell { row: self.row + 1, col: self.col + -1 },
            Cell { row: self.row + 0, col: self.col + -1 },
...
impl CellSet {
    pub fn count_occupied_adjacent_cells(&self, cell: Cell) -> usize {
        cell.adjacent_cells().into_iter().collect::<CellSet>().cells.intersection(&self.cells).count()
    }
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        s.lines().enumerate().flat_map(|(row, line)| {
            line.chars().enumerate().filter_map(move |(col, c)| {
                match c {
                    '.' => None,
                    '@' => Some(Ok(Cell { row: row as i32, col: col as i32 })),
...
fn test_the_cell_set_can_count_how_many_adjacent_cells_are_occupied() {
    assert_eq!( [].into_iter().collect::<CellSet>().count_occupied_adjacent_cells(Cell { row: 0, col: 0}), 0 );
    assert_eq!( [Cell{row:-1, col:0}].into_iter().collect::<CellSet>().count_occupied_adjacent_cells(Cell { row: 0, col: 0}), 1 );
    assert_eq!( [Cell{row:-2, col:0}].into_iter().collect::<CellSet>().count_occupied_adjacent_cells(Cell { row: 0, col: 0}), 0 );
}
...
fn test_we_can_translate_text_into_a_cell_set() {
    ...
    assert_eq!( ".".parse::<CellSet>() , Ok( CellSet::default() ));
    assert_eq!( "@".parse::<CellSet>() , Ok( [Cell{row:0, col:0}].into_iter().collect::<CellSet>() ));
    assert_eq!( "X".parse::<CellSet>() , Err( ParseCellSetError ));
    assert_eq!( ".@".parse::<CellSet>() , Ok( [Cell{row:0, col:1}].into_iter().collect::<CellSet>() ));
    assert_eq!( "..@@.@@@@.".parse::<CellSet>() , Ok( [
        Cell{row:0, col:2},
        Cell{row:0, col:3},
        Cell{row:0, col:5},
        Cell{row:0, col:6},
...
fn test_cell_set_can_count_accessible_rolls() {
    ...
    assert_eq!( "..@@.@@@@.".parse::<CellSet>().unwrap().count_accessible_rolls() , 6);
